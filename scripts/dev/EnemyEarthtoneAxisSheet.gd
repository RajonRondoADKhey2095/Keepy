extends Node
## Earth-tone colour RECON for the rat's (ENEMY) resting-state albedo,
...
##
## WHY THIS EXISTS -- the previous lot (EnemyGreyAxisSheet.gd) closed the
## NEUTRAL grey-white zone (S < 0.10): at that saturation hue is derived
## from an RGB spread too small to survive fog/AA/box-sampling, so no
## candidate separated from CHARGER on anything but a number. This
## script asks the next question, and it is deliberately the OPPOSITE
## one: does a REAL saturation (S ~ 0.15-0.25) buy a dependable
## separation while still reading as a grey-brown earth tone? The grey
## zone is NOT re-explored here -- it has an answer.
##
## ⚠️ TWO BANDS, NOT ONE -- and this is the whole shape of the problem.
## The ground renders at Lrel ~ 0.15, so clearing 3.0:1 against it means
## Lrel >= ~0.549 (BRIGHT band) or Lrel <= ~0.0165 (DARK band). Nothing
## in between clears, at any hue, at any saturation -- this is
## arithmetic on the WCAG formula, not a tuning preference, and it has
## been verified twice on this tree already. So the axis is explored in
## both bands at once:
##   BRIGHT -- candidates sit beside CHARGER (H=348, Lrel~0.595), which
##     already occupies the middle of this band. JUMP (H~43.6) and
##     STOMPER (H~202.3) are also here.
##   DARK -- candidates sit beside DODGE (H~8.1), the band's only
##     current occupant.
## Each candidate is therefore composed in the same frame as ITS OWN
## band-mate, never against an object from the other band: two objects
## in different luminance bands are trivially separable and comparing
## them would flatter every candidate.
##
## ⚠️ HSV SATURATION IS SCALE-INVARIANT -- THIS IS THE TRAP OF THE DARK
## BAND, and it is a DIFFERENT trap from the grey-axis one. A colour at
## V=0.12 with S=0.22 reports a perfectly respectable saturation, but
## its ABSOLUTE chroma (max-min of its RGB channels) is ~7/255. The
## grey axis failed because relative saturation was low; the dark band
## can fail because relative saturation is fine while absolute chroma is
## still near zero. Every row therefore prints `chroma8` -- the rendered
## max-min RGB spread on the 0..255 scale -- next to S, and the report
## must lean on chroma8 in the dark band, never on S alone.
##
## Three independent signals per candidate, same discipline as the grey
...
##      ratio against the band-mate -- the number that says whether the
##      two objects are separable by brightness at all.
...
##
## METHOD -- copied from EnemyGreyAxisSheet.gd (itself copied from
## EnemyColourSheetAxis.gd / DarkPaletteAudit.gd), not reinvented: a
## private calibration scene whose WorldEnvironment, DirectionalLight3D
## and ground material are READ OUT OF Game.tscn / TrackSegment.tscn at
## run time (PackedScene.get_state(), never hand-copied hex), a camera
## posed at CameraFollow's own steady-state transform, the REAL
## Obstacle.tscn instantiated twice, and the REAL SwampAtmosphere.gd
## driving the mist breath through GameState.mist_intensity so both ends
## of the breath are measured rather than assumed identical.
##
...
## single physics tick once GameState.state == PLAYING. This script
## needs the REST colour, so every time `_obstacle` is configured as
## ENEMY, `_enemy_settling` is forced back to false right after
## `configure()` -- which keeps `_physics_process` on its first
## early-return branch and never lets `_apply_enemy_alarm` touch the
## material again. This correction is carried over from the grey lot
## verbatim: without it the whole sheet measures the shared alarm red.
##
...
##     res://scripts/dev/EnemyEarthtoneAxisSheet.tscn
...
const HUE_RELIABLE_MIN_SAT: float = 0.10 # relative-saturation floor, inherited from the grey lot
## ABSOLUTE chroma floor, 0..255 scale. Independent of, and not
## interchangeable with, HUE_RELIABLE_MIN_SAT -- see class doc. A
## deliberately loose flagging aid, not a scientific threshold: it marks
## rows whose tint has fewer 8-bit levels of spread than a single
## quantisation step is wide on a phone panel's gamma curve.
...
const OUT_PNG_PATH := "res://docs/color-sheets/enemy_earthtone_axis_sheet.png"
...
## the band-mate in the composed shots (CHARGER for BRIGHT, DODGE for
...
## installs anything. Measured for real (no override) so the known
## CHARGER collision reproduces on this tree before anything is judged.
const ENEMY_SHIPPED_LABEL := "ENEMY shipped (pale pink, COLLIDES with CHARGER -- stays live, this lot installs nothing)"
...
## Green-yellow hues are favoured on purpose, and not for taste: the
## WCAG luminance weight on green is 0.7152 against 0.2126 on red and
## 0.0722 on blue, so the green side of the wheel reaches the band at a
## lower V (i.e. a less washed-out colour) than any other side. B3 is
## the deliberate control on the far side of the wheel to check that
## claim rather than assert it. Hues are held clear of CHARGER (348),
## JUMP (43.6) and STOMPER (202.3) -- beige/ochre, the obvious earth
## tone, is NOT tested because it sits on JUMP's hue.
const BRIGHT_CANDIDATES: Array = [
	{"label": "B1 Kaki pale (olive-vert, cote vert du cercle)", "h": 105.0, "s": 0.22, "v": 0.88},
	{"label": "B2 Sauge grisee (vert-gris, plus desature)", "h": 135.0, "s": 0.18, "v": 0.86},
	{"label": "B3 Mauve-brun pale (controle: cote oppose du cercle)", "h": 285.0, "s": 0.18, "v": 0.95},
]

## DARK band. Must render at Lrel <= ~0.0165, which forces V ~0.12 --
## so every candidate here is a near-black, and `chroma8` is the column
## that matters, not `sat`. Hues are held >45deg from DODGE (8.1):
## warm browns (H ~ 20-40), the most obviously "earthy" family, are
## therefore STRUCTURALLY UNAVAILABLE in this band -- they sit on
## DODGE's hue. D3 is placed at the closest hue this rule allows, to
## measure how much room the rule actually leaves.
const DARK_CANDIDATES: Array = [
	{"label": "D1 Brun-vert profond (terre humide)", "h": 110.0, "s": 0.22, "v": 0.115},
	{"label": "D2 Brun-violet profond (terre froide)", "h": 290.0, "s": 0.22, "v": 0.12},
	{"label": "D3 Terre d'ombre olive (hue la plus proche que la regle 45deg autorise)", "h": 65.0, "s": 0.24, "v": 0.12},
]
...
func _ready() -> void:
	...
	ProbeWatchdog.arm(self, "EnemyEarthtoneAxisSheet")
	_run.call_deferred()
...
func _run() -> void:
	...
	print("=== ENEMY (rat) EARTH-TONE recon -- real saturation (S~0.15-0.25), BOTH bands ===")
	...
	print("")
	print("--- ENEMY reference row (already known to collide with CHARGER -- stays live, untouched by this lot) ---")
	var shipped := await _measure_close(ENEMY_SHIPPED_LABEL, Obstacle.Type.ENEMY, null, true)
	shipped["band_mate"] = Obstacle.Type.keys()[Obstacle.Type.CHARGER]
	_results[ENEMY_SHIPPED_LABEL] = shipped
	_print_row(shipped)
	# The shipped rat gets a composed shot in the SAME format as every
	# candidate, and this is not decoration: it is the CALIBRATION of the
	# whole sheet. Without it, "B1 separates from CHARGER" is a claim
	# with nothing to be measured against; with it, the reader sees what
	# an already-agreed COLLISION looks like at the same distance, in the
	# same frame, and can judge every candidate against that, not against
	# an idea of what a collision might look like.
	# `null` here means "no override" -- the shipped row is composed with
	# the REAL installed albedo read off the material, never a hex copied
	# from the docs.
	await _capture_wide(ENEMY_SHIPPED_LABEL, null, Obstacle.Type.CHARGER)

	print("")
	print("--- BRIGHT BAND -- earth tones beside CHARGER (band-mate for the composed shot) ---")
	for c in BRIGHT_CANDIDATES:
		await _run_candidate(c, Obstacle.Type.CHARGER)
...
func _run_candidate(c: Dictionary, band_mate: Obstacle.Type) -> void:
	...
	rec["band_mate"] = Obstacle.Type.keys()[band_mate]
	_results[label] = rec
	_print_row(rec)
...
func _build_calibration_scene() -> void:
	...
	_ref_obstacle.configure(Obstacle.Type.CHARGER)
...
# =====================================================================
# COMPOSED SHOT -- candidate ENEMY (right lane) next to its OWN BAND-MATE
# (left lane), both at DECISION_Z, same camera, same frame. Never against
# the other band: two objects in different luminance bands separate
# trivially and would flatter every candidate.
# =====================================================================

## `candidate_color` may be null -- that means "no albedo override", i.e.
## compose the SHIPPED rat as it really is (used for the calibration row).
func _capture_wide(label: String, candidate_color, band_mate: Obstacle.Type) -> void:
	...
	_wide_crops[label] = _capture_crop([enemy_bounds, ref_bounds], WIDE_CROP_PAD_PX, WIDE_CROP_MAX_W, WIDE_CROP_MAX_H)
...
func _obstacle_sample_bounds(node: Obstacle, type: Obstacle.Type) -> AABB:
	...
	if slot == null:
		push_error("EnemyEarthtoneAxisSheet: no mesh node '%s' on Obstacle." % MESH_NODE_FOR_TYPE[type])
	...
	for n in _visual_descendants(slot):
		...
		any = true
	if not any:
		push_error("EnemyEarthtoneAxisSheet: mesh node '%s' draws nothing measurable." % MESH_NODE_FOR_TYPE[type])
		return AABB(node.global_position, Vector3.ONE * 0.1)
	return merged
...
func _sample_box(img: Image, cx: int, cy: int, half: int) -> Color:
	...
	for y in range(maxi(0, cy - half), mini(h, cy + half), step):
		for x in range(maxi(0, cx - half), mini(w, cx + half), step):
			...
			samples += 1
	if samples == 0:
		push_error("EnemyEarthtoneAxisSheet: sample box (%d,%d) half=%d off-screen." % [cx, cy, half])
		return Color(0, 0, 0)
	acc /= float(samples)
...
func _capture_crop(bounds_list: Array, pad_px: int, max_w: int, max_h: int) -> Image:
	...
	if x1 <= x0 or y1 <= y0:
		push_error("EnemyEarthtoneAxisSheet: crop rect degenerate, off-screen object.")
...
func _scene_environment(scene_path: String) -> Environment:
	...
	if env == null:
		push_error("EnemyEarthtoneAxisSheet: no WorldEnvironment.environment found in %s" % scene_path)
...
func _scene_directional_light(scene_path: String) -> DirectionalLight3D:
	...
	for i in state.get_node_count():
		...
		return light
	push_error("EnemyEarthtoneAxisSheet: no DirectionalLight3D found in %s" % scene_path)
	return light
...
func _scene_ground_material(scene_path: String) -> StandardMaterial3D:
	...
	if mat == null:
		push_error("EnemyEarthtoneAxisSheet: no ground surface_material_override/0 found in %s" % scene_path)
...
func _print_row(rec: Dictionary) -> void:
	...
	print("%-62s hue=%6.1f sat=%.3f chroma8=%5.1f val=%.2f lum=%.5f | shallow=%.2f:1 deep=%.2f:1 worst=%.2f:1" % [
	...
	print("    dHue vs C/J/S/D  = %s / %s / %s / %s deg  (floor %.0fdeg)" % [
		_fmt_dist(d_charger), _fmt_dist(d_jump), _fmt_dist(d_stomper), _fmt_dist(d_dodge), HUE_MIN_SEPARATION,
	])
...
func _flag_string(rec: Dictionary, d_c, d_j, d_s, d_d, _l_c) -> String:
...
func _print_verdict_block() -> void:
	...
	print("%-62s %-8s %-9s %-9s %-11s %s" % ["candidat", "worst", "chroma8", "dHue min", "paire", "band-mate"])
	for arr in [BRIGHT_CANDIDATES, DARK_CANDIDATES]:
		for c in arr:
			...
			var dmin := 9999.0
			for ref_label in ["CHARGER", "JUMP", "STOMPER", "DODGE"]:
...
func _compose_png() -> void:
	...
	var section_breaks := {
		4: "ENEMY REFERENCE ROW -- le rose installe, deja en collision avec CHARGER. Reste live: ce lot n'installe rien.",
		5: "BANDE CLAIRE (Lrel >= ~0.549) -- chaque candidat compose avec le vrai CHARGER, meme frame, z=%d" % int(DECISION_Z),
	...
	# IN THE CONTROL ABOVE. Same reason as EnemyGreyAxisSheet.gd /
	# EnemyColourSheetAxis.gd: in this sandbox's software GL stack,
	# draw_texture_rect() on an ImageTexture built from a captured
	# viewport crop rendered flat WHITE, while draw_rect() fills were
	# never affected. Sidestepping the 2D texture-draw path entirely
	# avoids the whole class of bug. DO NOT "simplify" this back.
	...
	if err != OK:
		push_error("EnemyEarthtoneAxisSheet: save_png failed with error %d" % err)
	else:
...
class EarthtoneAxisCanvas:
	...
	func _draw() -> void:
		...
		draw_string(font, Vector2(20, 30), "ENEMY (rat) -- AXE GRIS-BRUN TERREUX a saturation reelle (S~0.15-0.25), les DEUX bandes",
		...
		draw_string(font, Vector2(20, 124), "La zone gris NEUTRE (S<0.10) a ete fermee au lot precedent et n'est pas re-exploree ici. Le rose installe reste live: ce lot n'installe rien.",
		...
		for i in order.size():
			...
			lines.append("contraste vs sol: shallow=%.2f:1  deep=%.2f:1  worst(gate)=%.2f:1  (plancher 3.0:1, sur 3.05:1)" % [
				rec.shallow_ratio, rec.deep_ratio, rec.worst_ratio])
			lines.append("dHue -> C/J/S/D = %s/%s/%s/%s deg  (plancher 45)" % [
				_fmt(d_c), _fmt(d_j), _fmt(d_s), _fmt(d_d)])
			lines.append("dLum -> C/J/S/D = %s/%s/%s/%s  (luminance relative, 0..1)" % [
			...
			if wide_row_y.has(label):
				...
				draw_string(font, Vector2(20, wy - 6), "Compose a distance de decision (z=-12): candidat ENEMY (voie droite) a cote du vrai %s (voie gauche) -- meme bande, meme frame" % mate,
